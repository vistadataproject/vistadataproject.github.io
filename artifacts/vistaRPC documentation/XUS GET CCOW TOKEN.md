...
## MUMPS Method description
 ...
 Code | ```  N HDL,HDL2,X<br/> S RET(0)="NO PROXY USER",RET(1)="ERROR"<br/> I $$USERTYPE^XUSAP(DUZ,"APPLICATION PROXY") Q  ;No Proxy<br/> I $$USERTYPE^XUSAP(DUZ,"CONNECTOR PROXY") Q  ;No Proxy<br/> S RET(0)="NOT AUTHENTICATED",RET(1)="ERROR"<br/> I $G(DUZ("LOA"))=1 Q  ;Not an authenticated user<br/> S X=$$ACTIVE^XUSER(DUZ) I 'X S RET(0)=X Q  ;User must be active<br/> S HDL=$$HANDLE("XWBCCW",1)<br/> S RET(0)="~2"_$$LOW^XLFSTR(HDL),RET(1)=$G(^XMB("NETNAME"))_"^"_$$STA^XUAF4(DUZ(2))<br/> D TOK(HDL)<br/> S ^XUTL("XQ",$J,"HDL")=HDL ;Save handle with job```{::nomarkdown} <br/><br/><p style="font-size: 11px">Generated on January 14th 2017, 7:36:25 am</p>{:/}