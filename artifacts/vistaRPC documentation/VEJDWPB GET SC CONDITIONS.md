...
## [VISTA RPCs](TableOfContent.md) &#8594; VEJDWPB GET SC CONDITIONS 

 property | value 
--- | --- 
 label | VEJDWPB GET SC CONDITIONS
 tag | SCCOND
 routine | [VEJDVST](http://code.osehra.org/dox/Routine_VEJDVST_source.html)
 return value type | SINGLE VALUE
 description | This will return a string indicating whether the service conditionquestions need to be asked:   Agent Orange exposure   Environmental Contaminant exposure   Service Connected   Military Sexual Trauma   Head & Neck Cancer If VST is not passed, then APPT must be the scheduling appointmentdate/time and LOC must be the the hospital location data If VST is passed, then APPT and LOC are optional.
