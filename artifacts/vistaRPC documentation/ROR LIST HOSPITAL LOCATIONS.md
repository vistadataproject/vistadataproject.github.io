...
| vs:Input_Parameter-8994_02 | HLTYPES | LITERAL |  |  | List of location types separated by commas (internal values of the TYPEfield of the HOSPITAL LOCATION file):   C   Clinic  F   File Area  I   Imaging  M   Module  N   Non-Clinic Stop  OR  Operating Room  W   Ward  Z   Other Location Only locations of the types defined by this parameter are selected by the remote procedure. By default ($G(HLTYPES)=\\), all locations are selected. | 
| vs:Input_Parameter-8994_02 | DIVIEN | LITERAL |  |  | Division IEN. If this parameter is defined and greater than zero then onlythe locations associated with this division are selected. | 