...
--- | --- | --- | ---
EN1^GMVDCSAV | SINGLE VALUE<br><br>RESULT does not return a value.  The data is filed in the GMRV VITAL MEASUREMENT (#120.5) file. Example:  > S GMRVDATA="3051011.1635^134^1;120/80;^67^87*2:38:50:75" > D EN1^GMVDCSAV(.RESULT,GMRVDATA) | SUBSCRIPTION
...
Function | Return Value | Availability
--- | --- | --- | ---
CHKDEV^GMVUTL2 | ARRAY<br><br>RESULT(n)=P1^P2^P3^P4^P5^P6  where n = a sequential number starting with 1      P1 = File 3.5  IEN      P2 = File 3.5 name (.01 value)       P3 = File 3.5 name (.01 value) or flag to indicate last entry in            the array      P4 = location of terminal      P5 = right margin      P6 = page length | RESTRICTED
...
Function | Return Value | Availability
--- | --- | --- | ---
CLINPTS^GMVCLIN | ARRAY<br><br>Returns a list of patient names and DFNs for the selected clinic and thegiven date span in the array specified. | RESTRICTED
...
Function | Return Value | Availability
--- | --- | --- | ---
GETDT^GMVGETQ | SINGLE VALUE<br><br>RESULT=Date in internal FileMan format^Date in external FileMan format Example: > S GMRDATE="10/11/2005@10:30AM" > D GETDT^GMVGETQ(.RESULT,GMRDATE) ZW RESULT > RESULT="3051011.103^OCT 11, 2005@10:30:00" | SUBSCRIPTION
...
Function | Return Value | Availability
--- | --- | --- | ---
DLL^GMVUTL8 | SINGLE VALUE<br><br>Returns YES if the file can be used. Returns NO, if the file cannot beused. Returns null if the file was not found. Example: > S GMVX="GMV_VITALSVIEWENTER.DLL:v. 07/21/05 10:34" > D DLL^GMVUTL8(.RESULT,GMVX) ZW RESULT > RESULT="NO" | SUBSCRIPTION
...
Function | Return Value | Availability
--- | --- | --- | ---
EN1^GMVER0 | SINGLE VALUE<br><br>Returns a message stating the outcome of the request to queue the report. If the report was successfully queued, RESULT will be "Report sent to device. Task #: " ZTSK" where ZTSK is the task number of the job. If the report could not be queued, RESULT will be "Unable to task the report." | RESTRICTED
...
Function | Return Value | Availability
--- | --- | --- | ---
CATEGORY^GMVUTL8 | SINGLE VALUE<br><br>Returns the IEN if GMVCAT exists in FILE 120.53 Example: > S GMVCAT="METHOD" > D CATEGORY^GMVUTL8(.RESULT,GMVCAT) ZW RESULT > RESULT=2 | SUBSCRIPTION
...
Function | Return Value | Availability
--- | --- | --- | ---
TYPE^GMVUTL8 | SINGLE VALUE<br><br>Returns the IEN if GMVTYPE exists in FILE 120.51. Example: > S GMVTYPE="WEIGHT" > D TYPE^GMVUTL8(.RESULT,GMVTYPE) ZW RESULT > RESULT=9 | SUBSCRIPTION
...
Function | Return Value | Availability
--- | --- | --- | ---
ERROR^GMVUTL1 | SINGLE VALUE<br><br>If the record is marked as entered in error, RESULT is set to "OK".Otherwise, RESULT is set to "Record Not Found"  Example: > S GMVDATA="1560^547^1" > D ERROR^GMVUTL1(.RESULT,GMVDATA) ZW RESULT > RESULT="OK" | SUBSCRIPTION
...
Function | Return Value | Availability
--- | --- | --- | ---
APTLIST^GMVUTL8 | ARRAY<br><br>ARRAY - Subscripted by sequential number with DFN in first piece andpatient name in second piece.  example: ARRAY(#)=DFN^patient name^SSN^DOB^SEX AND AGE^ATTENDING^VETERAN          ^INTERNAL DATE/TIME DECEASED^EXTERNAL DATE/TIME DECEASED | RESTRICTED
...
Function | Return Value | Availability
--- | --- | --- | ---
EN1^GMVSR0 | SINGLE VALUE<br><br>Returns a message stating the outcome of the request to queue the report.If the report was successfully queued, RESULT will be "Report sent to device. Task #: " ZTSK" where ZTSK is the task number of the job. If the report could not be queued, RESULT will be "Unable to task the report." | RESTRICTED
...
Function | Return Value | Availability
--- | --- | --- | ---
RPC^GMVRPCP | GLOBAL ARRAY<br><br>RESULTS(0)   =Success or failure flag (-1 or 1) from both SELECT & LOGSEC               methodsRESULTS(1..n)=Messages to process by the client from the SELECT method. | RESTRICTED
...
Function | Return Value | Availability
--- | --- | --- | ---
EN1^GMVCAQU | GLOBAL ARRAY<br><br>Returns a message stating the outcome of the request to queue the report. If the report was successfully queued, RESULT will be "Report sent to device. Task #: " ZTSK" where ZTSK is the task number of the job. If the report could not be queued, RESULT will be "Unable to task the report." | RESTRICTED
...
Function | Return Value | Availability
--- | --- | --- | ---
ROOMBED^GMVGETD | GLOBAL ARRAY<br><br>Returns the global array name (i.e., ^TMP($J,"GROOM")) containing a listof rooms/beds for the given MAS ward. ^TMP($J,"GROOM",n)=Roombed n is a sequential number starting at 1. If there is no data, then the global array is undefined. | RESTRICTED
...
Function | Return Value | Availability
--- | --- | --- | ---
TEAMPT^GMVUTL3 | ARRAY<br><br>Returns a list of patients in the array specified. RESULT(n)=Patient name^DFN^SSN (w/hyphens)^DOB (external)^SEX and AGE^          Attending^Veteran^Date of Death (external)^Date of Death           (internal)^Ward name^Roombed n is a sequential number starting at 1. | RESTRICTED
...
Function | Return Value | Availability
--- | --- | --- | ---
ROOMPT^GMVUTL7 | ARRAY<br><br>RESULT(n)=patient name^DFN^DOB (external)^SSN (no hyphens) where n is a sequential number beginning with 0 (zero) | RESTRICTED
