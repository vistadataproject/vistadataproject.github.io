---
layout: default
title: VISTA RPC Documentation
---

#### [Developer Documentation](../index) &#187; [Native RPCs](TableOfContents) &#187; ORWDXM1 SVRPC<br/>
# ORWDXM1 SVRPC



## Properties

Property | Value
--- | ---
Label | SVRPC
MUMPS Implementation | [ORWDXM1](http://code.osehra.org/dox/Routine_ORWDXM1_source.html)
Return Type | SINGLE VALUE




## MUMPS Method Description

Property | Value
--- | ---
Method | [SVRPC^ORWDXM1](http://code.osehra.org/dox/Routine_ORWDXM1_source.html)
Method Comment | RPC FOR SITEVAL
Input Parameters | X
Code | {::nomarkdown}<pre><code> S RET=$$SITEVAL</code></pre>{:/}



## CPRS

File Type | Description
--- | ---
Pascal File | {::nomarkdown}<a href="https://github.com/OSEHRA/VistA/blob/master/Packages/Order%20Entry%20Results%20Reporting/CPRS/CPRS-Chart/Orders/rODRad.pas">Orders/rODRad.pas</a>{:/}

{::nomarkdown} <br/><p style="font-size: 11px">Document generated on August 24th 2017, 2:57:41 pm</p>{:/}