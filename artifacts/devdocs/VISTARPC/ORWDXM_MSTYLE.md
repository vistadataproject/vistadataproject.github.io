---
layout: default
title: VISTA RPC Documentation
---

#### [Developer Documentation](../index) &#187; [Native RPCs](TableOfContents) &#187; ORWDXM MSTYLE<br/>
# ORWDXM MSTYLE

Return the menu style for the system.

## Properties

Property | Value
--- | ---
Label | MSTYLE
MUMPS Implementation | [ORWDXM](http://code.osehra.org/dox/Routine_ORWDXM_source.html)
Return Type | SINGLE VALUE




## MUMPS Method Description

Property | Value
--- | ---
Method | [MSTYLE^ORWDXM](http://code.osehra.org/dox/Routine_ORWDXM_source.html)
Method Comment | Return the menu style for the system
Code | {::nomarkdown}<pre><code> S VAL=+$$GET^XPAR("SYS","ORWDXM ORDER MENU STYLE",1,"I")</code></pre>{:/}



## CPRS

File Type | Description
--- | ---
Pascal File | {::nomarkdown}<a href="https://github.com/OSEHRA/VistA/blob/master/Packages/Order%20Entry%20Results%20Reporting/CPRS/CPRS-Chart/Orders/rODBase.pas">Orders/rODBase.pas</a>{:/}

{::nomarkdown} <br/><p style="font-size: 11px">Document generated on August 24th 2017, 2:57:41 pm</p>{:/}