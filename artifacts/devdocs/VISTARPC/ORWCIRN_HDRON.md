---
layout: default
title: VISTA RPC Documentation
---

#### [Developer Documentation](../index) &#187; [Native RPCs](TableOfContents) &#187; ORWCIRN HDRON<br/>
# ORWCIRN HDRON

Get parameter value for ORWRP HDR ON

## Properties

Property | Value
--- | ---
Label | HDRON
Routine | [ORWCIRN](http://code.osehra.org/dox/Routine_ORWCIRN_source.html)
Return Type | SINGLE VALUE




## MUMPS Method Description

Property | Value
--- | ---
Method | [HDRON^ORWCIRN](http://code.osehra.org/dox/Routine_ORWCIRN_source.html)
Method Comment | Get parameter value for ORWRP HDR ON
Code | {::nomarkdown}<pre><code> S ORY=+$$GET^XPAR("ALL","ORWRP HDR ON",1,"I")<br/></code></pre>{:/}



## CPRS

File Type | Description
--- | ---
Pascal File | {::nomarkdown}<a href="https://github.com/OSEHRA/VistA/blob/master/Packages/Order%20Entry%20Results%20Reporting/CPRS/CPRS-Chart/rReports.pas">rReports.pas</a>{:/}

{::nomarkdown} <br/><p style="font-size: 11px">Document generated on July 13th 2017, 2:13:30 pm</p>{:/}