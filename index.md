...
Now, for the first time, VA's native transactional healthcare data model - the VISTA Data Model -  will be comprehensively exposed, enriched, and operationalized as a single, secure, symmetric read-write, server-side interface for all clinical operational VISTA data. The VISTA Data Model is in turn normalized across all local VISTA system data models to create a national, standardized __Master Veteran Data Model__ (MVDM), alowing read-write transactions across all VA VISTA systems using a single, standard, secure, veteran-centric, service-based mechanism, MVDM Services: 
