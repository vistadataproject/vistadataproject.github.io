...
Now, for the first time, VISTA's true, native, transactional data model -  the VISTA Data Model (VDM)- will be comprehensively exposed, enriched, and operationalized as a secure, symmetric (bidirectional), server-side interface to all VISTA data.  

This VISTA Data Model is in turn normalized across all local VISTA systems creating an enterprise __Master VISTA Data Model (MVDM__), which allows secure read-write transactions to all 131 VISTA systems throughout the enterprise with one single, standard, secure mechanism.


##  VISTA's new SAFE Interface
Current VA VISTA interfaces, both new and legacy, wrap legacy procedural code and remote procedure calls (RPCs) within various __mid-tier__ object models  __above the RPCs__ (figure below, left). This dependency on and encapsulation of RPCs within the mid-tier model not only fails to remediate, but propagates forward all the problems inherent with the legacy MUMPS code and RPCs - most notably lack of auditing and security.
