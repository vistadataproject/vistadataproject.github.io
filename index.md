...
In the VISTA Data Project (VDP), this VISTA Data Model (VDM) is comprehensively represented and then normalized to produce a national, standardized Master Veteran Data Model (MVDM). An operationalized MVDM provides new web clients with a single, secure, symmetric read-write interface for every VISTA and through _emulation_ of existing interfaces, VISTA clients such as CPRS run unchanged but with greatly enhanced security. 
...
In _VDP_, Data Modeling starts with 100% coverage of all VISTA data in web-standard JSON and moves up to a normalized master model for the functionality used by CPRS, VISTA's client, and provides new web clients with a modern, symmetric (read and write share the same data format), read-write interface ...
