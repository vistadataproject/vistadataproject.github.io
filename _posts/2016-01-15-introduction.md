...
__Data Linkage__  | __MVDM provides VISTA with capability to natively, securely link to structured, computable data resources on the web, dramatically enriching VISTA's data.__ Linked Data is used by the largest web search engines (including Google, Yahoo, and Microsoft) to search, link and structure all data on the web. Because MVDM uses the same Linked Data model and technology as these world's largest web search organizations, VISTA inherits the capability to securely link to these millions of external structured data sources in computable form.  Linking specific patient information from VISTA, for example, would allow highly personalized enrichment of the veteran information with always-up-to-date authoritative sources of information.
